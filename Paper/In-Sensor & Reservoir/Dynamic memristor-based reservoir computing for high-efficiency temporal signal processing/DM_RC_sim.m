...
clear;
% ----------------------Model Parameters----------------------
% para.r, para.G0, para.Kp, para.Kn, para.alpha 等代表记忆阻器模型中的各种常数
para.r = 0.99; % 衰减因子
para.G0 = 0.5; % 初始电导
para.Kp = 9.13; % 正向调制因子
para.Kn = 0.32; % 负向调制因子
...
% ----------------------Voltage Sequence----------------------
% V 是一个从 0 到 +3 V 再到 -3 V 再回到 0 V 的往复电压序列，用 -[...] 来翻转和拼接
Vmin = -3;
Vmax = 3;
inv = 0.099; %步长
...
% ----------------------Start Simulation----------------------
% 每次调用 DynamicMemristor(V(i), G, para) 来更新电流和电导
step = length(V);
I = zeros(1, step);
G = para.G0;
for i = 1:step
...
% ----------------------DATASET----------------------
sample = 8; %每个基准波形长度为 8。
step = 2000; %时间步长
Data = zeros(1, 2*step);
p1 = sin(pi*2*(0:sample-1)/sample); %长度为8的正弦波
p2(1:sample/2) = 1; %长度为8的方波（前半段为 +1，后半段为 -1）
p2(sample/2+1:sample) = -1;
% 通过随机方式把这两个波形混合生成长度为 2*step=4000 的 Data，
% 对应标签存放在 Label 中（正弦波记为 0，方波记为 1）
...
% mask process
Mask = 2*unidrnd(2, N, ML)-3; % 生成一个 N×ML 的随机 ±1 矩阵
Input_ex = [];
for j = 1:N
    for i = 1:step
...
for i = 1:length(Input_ex(1, :))
    [memout(:,i), G] = DynamicMemristor(Input_ex(:,i), G, para); %返回第i步的N个节点输出电流和电导
