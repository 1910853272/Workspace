...
%%
sc3_cover_logic = (uz3<800) & (z3~=0) & (ux3<509);
...
% fit poly11
[fitobject_sc3,gof_sc3] = fit([tofit_x3,tofit_y3],tofit_z3,'poly11','Robust','Bisquare');
fitcoeffvalue_sc3 = coeffvalues(fitobject_sc3);

%% plot fit plane in world coordinate
curvefit_b_world_sc3 = [0.771637535694371	0.0318797625757312	-0.0122080879704422];
b_sc3 = curvefit_b_world_sc3;
x3fit = linspace(-0.7,0.7,600);
y3fit = linspace(-0.7,0.7,600);
[X3FIT,Y3FIT] = ndgrid(x3fit,y3fit);
Z3FIT = b_sc3(1) + b_sc3(2)*X3FIT + b_sc3(3)*Y3FIT;
X3FIT_vector = reshape(X3FIT,[length(x1fit)*length(y1fit) 1]);
Y3FIT_vector = reshape(Y3FIT,[length(x1fit)*length(y1fit) 1]);
Z3FIT_vector = reshape(Z3FIT,[length(x1fit)*length(y1fit) 1]);
