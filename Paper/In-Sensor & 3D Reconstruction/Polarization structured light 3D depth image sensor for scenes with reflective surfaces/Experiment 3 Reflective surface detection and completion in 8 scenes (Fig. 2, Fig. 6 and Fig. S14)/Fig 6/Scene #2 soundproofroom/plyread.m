...
cy = 396.714142 / 2;
factor = 1000;
% sc1 convert to camera coordinate
uz1 = z1*factor;
ux1 = x1*fx./z1 + cx;
uy1 = y1*fy./z1 + cy;
% sc2 convert to camera coordinate
uz2 = z2*factor;
ux2 = x2*fx./z2 + cx;
uy2 = y2*fy./z2 + cy;
% sc3 convert to camera coordinate
uz3 = z3*factor;
ux3 = x3*fx./z3 + cx;
uy3 = y3*fy./z3 + cy;
...
logical_idmap = (map>0.99); 
logical_idmap2 = (map2>0.99);
logical_idmap = (logical_idmap) & (mapX1FIT < 320);
logical_idmap2 = (logical_idmap2) & (map_x_vector < 320);
...
temp_ux1 = ux1;
temp_uy1 = uy1;
logic_id_cam = (tempuz<300) | (substract_uz>800) | (~logical_erode_id_inmap);
...
[fitobject,gof] = fit([tofit_x,tofit_y],tofit_z,'poly11','Robust','Bisquare');
fitcoeffvalue = coeffvalues(fitobject);

%% plot the fitplane in world coordinate
curvefit_b_world = [0.631927529735470	0.0716286546747409	0.0782779672472915];
b = curvefit_b_world;
x1fit = linspace(-0.6,0.2,400); % big enough to cover the glass region
y1fit = linspace(-0.4,0.4,400);
[X1FIT,Y1FIT] = ndgrid(x1fit,y1fit);
Z1FIT = b(1) + b(2)*X1FIT + b(3)*Y1FIT;
X1FIT_vector = reshape(X1FIT,[160000 1]);
Y1FIT_vector = reshape(Y1FIT,[160000 1]);
Z1FIT_vector = reshape(Z1FIT,[160000 1]);
...
fitplane_camrgb_y = prgb(:,2) * colorfx ./ prgb(:,3) + colorcy;
...
mykky = mapY1FIT;
mykkz = myfitz;
mykkx(~logical_idmap) = nan;
mykky(~logical_idmap) = nan;
mykkz(~logical_idmap) = nan;
...
%% remove points which are outside the view
map_outside_depthBoundary_id = (filled_map_uy<5) | (filled_map_uy>395) | ...
    (filled_map_ux<5) | (filled_map_ux>619);
