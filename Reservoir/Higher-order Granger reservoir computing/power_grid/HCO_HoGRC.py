...
def _initialize_input_weights(args,edges_in,edges_out,edges_ex):
    ...
    for ni in range(args.n):
        ...
        for edge in edges_out:
            #if edge[0] == ni:
            if edge[0] == ni and ([edge[0],edge[1]] not in edge_d):
                for ex in edges_ex:
...
def sav_er1(preds2s,preds3s,error2s,error3s): 
    for ni in range(args.n):
        for vj in range(args.V):
            ...
            error_2pd.to_csv('results/error88m'+str(ni)+'_'+str(vj)+'_2.csv')
            error_3pd.to_csv('results/error88m'+str(ni)+'_'+str(vj)+'_3.csv')
...
def sav_er2(preds2s,preds3s,error2s,error3s):
    ...
    s2.to_csv('results/pred2s88m_2.csv')
    s3.to_csv('results/pred3s88m_3.csv')
